...
JVM.prototype.run = function() {
        ...
        if (!this.entryPoint.className || (this.entryPoint.className === classArea.getClassName())) {    
            if ((classArea.getAccessFlags() & ACCESS_FLAGS.ACC_PUBLIC) !== 0) {
                ...
                for(var i=0; i<methods.length; i++) {
                    ...
                    {
                        entryPointFrame = new Frame(classArea, methods[i]);    
                    }
                }
            }
        }
    ...
    process.exit(
        this.runFrame(entryPointFrame).apply(null, arguments)
    );
    
}

JVM.prototype.runFrame = function(frame) {
    var self = this;
        ...
        while(true) {
            ...
            switch(cmd) {
                ...
                case Opcodes.invokestatic:
                    ...
                    var aNewFrame = self.getNewFrame(packageClassName, method);
                    
                    if (aNewFrame) {                    
                        var res = self.runFrame(aNewFrame).apply(null, args);
